...
import com.github2136.base.download.entity.DownloadBlock
 ...
 */
class DownloadTaskHttp(val app: Application, private val url: String, private val filePath: String, val callback: (state: Int, progress: Int, path: String, url: String, error: String?) -> Unit) {
    ...
    private fun download(fileId: Long, threadSize: Int, url: String, contentLength: Long) {
        ...
        for (i in 0 until threadSize) {
            ...
            thread {
                ...
                try {
                    val conn = URL(url).openConnection() as HttpURLConnection
                    conn.addRequestProperty("RANGE", "bytes=${fileBlock.start + fileBlock.fileSize}-${fileBlock.end}")
                    conn.connectTimeout = 10000
